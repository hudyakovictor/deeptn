...
from ..shared.schemas import ForensicVerdict, Hypothesis, PairEvidence, Stage1Record, Stage2Record, Stage2IdentityRecord, Stage4Record
...
class VerdictEngine:
    ...
    def build_verdicts(self, main_root: str | Path) -> tuple[list[Stage4Record], list[dict[str, object]]]:
        root = Path(main_root)
        ...
        verdicts: list[Stage4Record] = []
        timeline: list[dict[str, object]] = []
        
        total = len(stage2_records)
        log.info(f"Building verdicts for {total} photos")
        
        error_count = 0
        
        # Progress bar
        progress = create_progress(total=total, description="S5 Verdict")
        ...
        for record in stage2_records.values():
            ...
            try:
                progress.update(photo_id=photo_id, status="classifying")
                ...
                error_count += 1
                log.error(f"[{photo_id}] Verdict failed: {exc}")
                progress.update(photo_id=photo_id, status=f"FAILED: {exc}", error=True)
                progress.advance()
                continue  # Continue with next photo
        ...
        # Calibration report
        if verdicts:
            y_true = np.array([1 if v.verdict.hypothesis.value == "H0_SAME" else 0 for v in verdicts])
            y_prob = np.array([v.verdict.posterior.get("H0_SAME", 0.5) for v in verdicts])
        ...
        # Summary
        if error_count > 0:
            log.error(f"S5 complete: {len(verdicts)}/{total} verdicts, {error_count} errors")
        else:
            log.success(f"S5 complete: {len(verdicts)}/{total} verdicts ✓")
    ...
    def _load_stage3_records(self, root: Path) -> dict[str, Stage2IdentityRecord]:
        records: dict[str, Stage2IdentityRecord] = {}
        for path in sorted(root.glob("*/identity.json")):
            payload = load_json(path)
            if payload:
                record = Stage2IdentityRecord.model_validate(payload)
                records[record.photo_id] = record
        return records
    ...
    def _verdict_for_photo(
        ...
        stage3: Stage2IdentityRecord | None,
        ...
        # FIX #2: Empty pairs → use neutral defaults (not 0.0 which biases H0)
        # When no pairs exist, geometry/texture distance is UNKNOWN, not "perfect match"
        _NEUTRAL_GEOM = 1.0   # Neutral geometry distance (neither same nor different)
        _NEUTRAL_TEX = 0.5    # Neutral texture distance
        _NEUTRAL_SUS = 0.5    # Neutral suspicion
        avg_geom = float(np.mean([p.geometry_distance for p in pairs])) if pairs else _NEUTRAL_GEOM
        avg_tex = float(np.mean([p.texture_distance for p in pairs])) if pairs else _NEUTRAL_TEX
        avg_same = float(np.mean([p.same_suspicion for p in pairs])) if pairs else _NEUTRAL_SUS
        avg_diff = float(np.mean([p.different_suspicion for p in pairs])) if pairs else _NEUTRAL_SUS
        avg_syn = float(np.mean([p.synthetic_suspicion for p in pairs])) if pairs else _NEUTRAL_SUS
        ...
        _has_pairs = bool(pairs)  # Track whether pairs exist for downstream logic
        quality = float(stage1.quality.overall_quality) if stage1 else 0.5
        # FIX #3: quality_penalty was INVERTED — low quality gave HIGH penalty
        # which INCREASED H1 likelihood. Now: high quality = high confidence factor.
        # quality_factor: 1.0 at quality=1.0, 0.8 at quality=0.0
        quality_factor = float(np.clip(0.8 + 0.2 * quality, 0.8, 1.0))
        texture_unreliable = bool(record.quality_summary.get("texture_unreliable", False))
        # FIX #8: Default identity_hint to "UNKNOWN" (not "PUT") when stage3 missing.
        # "PUT" gives identity_boost=0.12 which biases H0 for photos without identity data.
        identity_hint = stage3.identity_hint if stage3 else "UNKNOWN"
        ...
        for k, v in record.metric_notes.items():
            if k.startswith("physical_"):
                try:
                    ...
                    pass
        # FIX #5: spectral_slope was UNNORMALIZED (values 2-4, * 0.15 = 0.3-0.6 boost)
        # FIX #6: sss_index can be NEGATIVE (-1 to 0), which reduced H1 unexpectedly
        # Now: spectral_slope normalized to [0,1] range, sss_index clipped to [0,1]
        _raw_spectral = phys.get("spectral_slope", 2.5)
        _norm_spectral = float(np.clip((_raw_spectral - 2.0) / 2.0, 0.0, 1.0))  # Map 2-4 → 0-1
        _raw_sss = phys.get("sss_index", 0.0)
        _clip_sss = float(np.clip(_raw_sss, 0.0, 1.0))  # Only positive SSS contributes
        ...
        likelihoods = {
            ...
            Hypothesis.H1_SYNTHETIC.value: max(1e-6, np.clip((avg_tex * 0.6 + avg_syn * 0.5 + silicone_boost + silicone_physical_boost * 0.35 + h1_result.get("h1_probability", 0.0) * 0.4) * quality_factor, 0.01, 2.0)),
        ...
        # FIX #9: Cap cumulative multipliers to prevent excessive H1 boost.
        # Previously: 1.18 * 1.25 * 1.20 = 1.77× H1 boost when all flags fire.
        # Now: track cumulative multiplier and cap at 1.35× for H1 and 0.65× for H0.
        _h1_cumul = 1.0
        _h2_cumul = 1.0
        _h0_cumul = 1.0
        _hu_cumul = 1.0
        _MAX_BOOST = 1.35   # Max cumulative boost to any hypothesis
        _MAX_REDUCE = 0.65  # Min cumulative reduction
        ...
        _h1_cumul = float(np.clip(_h1_cumul, _MAX_REDUCE, _MAX_BOOST))
        _h2_cumul = float(np.clip(_h2_cumul, _MAX_REDUCE, _MAX_BOOST))
        _h0_cumul = float(np.clip(_h0_cumul, _MAX_REDUCE, _MAX_BOOST))
        _hu_cumul = float(np.clip(_hu_cumul, _MAX_REDUCE, _MAX_BOOST))
        
        likelihoods[Hypothesis.H1_SYNTHETIC.value] *= _h1_cumul
        likelihoods[Hypothesis.H2_DIFFERENT.value] *= _h2_cumul
        likelihoods[Hypothesis.H0_SAME.value] *= _h0_cumul
        likelihoods[Hypothesis.H_UNCERTAIN.value] *= _hu_cumul
    ...
    def _compute_alpha_clusters(
        ...
        for photo_id, s1 in stage1_records.items():
            try:
                ...
                error_count += 1
                log.error(f"[{photo_id}] Failed to load alpha data: {exc}")
        
        if error_count > 0:
            log.error(f"Alpha data: {len(records_with_alpha)} loaded, {error_count} failed")
        ...
        try:
            ...
            log.error(f"Alpha clustering failed: {exc}")
            return None
    ...
    def _check_biological_constraints(
        ...
        for other_id, other_stage1 in stage1_records.items():
            ...
            try:
                ...
                log.error(f"[{record.photo_id}↔{other_id}] Biological check failed: {exc}")
