...
from ..s1_extraction.metrics.cross_modal_rules import CrossModalTextureRules
...
class H1SyntheticDetector:
    ...
    def _era_from_year(self, year: int) -> str:
        if year < 2012:
            return "early_scan"
        elif year < 2021:
            return "udmurt_era"
    ...
    def evaluate(self, photo: Dict, anchor_comparison: Dict,
        ...
        cross = self.cross_modal.evaluate(
            ...
            self._era_from_year(photo.get("year", 2000)),
        )
        if cross:
            evidence.extend(cross)
            # FIX #10: Removed * 2 multiplier — cross-modal evidence should not be doubled
            score += sum(e.get("h1_boost", 0) for e in cross)
