...
from ..shared.schemas import CalibrationReference, PipelineDataset, Stage1Record, Stage2Record, Stage2IdentityRecord
...
class CalibrationEngine:
    ...
    def build_reference(self, calibration_root: str | Path) -> CalibrationReference | None:
        ...
        if not records:
            log.warning("Нет calibration stage2 записей в %s", calibration_root)
            return None
        ...
        log.info(f"Calibration health: {health_summary}")
    ...
    def annotate_main_dataset(self, main_root: str | Path, reference: CalibrationReference) -> list[Stage2IdentityRecord]:
        root = Path(main_root)
        records = self._load_stage2_records(root)
        stage1_records = self._load_stage1_records(root)
        
        total = len(records)
        log.info(f"Annotating {total} photos with identity hints")
        
        annotated: list[Stage2IdentityRecord] = []
        error_count = 0
        
        # Progress bar
        progress = create_progress(total=total, description="S2 Identity")
        ...
        for record in records:
            ...
            try:
                progress.update(photo_id=photo_id, status="annotating")
                ...
                error_count += 1
                log.error(f"[{photo_id}] Annotation failed: {exc}")
        ...
        # Summary
        if error_count > 0:
            log.error(f"S3 complete: {len(annotated)}/{total} annotated, {error_count} errors")
        else:
            log.success(f"S3 complete: {len(annotated)}/{total} annotated ✓")
