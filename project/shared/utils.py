...
def list_images(path: str | Path) -> list[Path]:
    ...
    if not root.exists():
        return []
    # Pipeline artifacts to exclude (generated by previous runs)
    _EXCLUDE_STEMS = {"face_crop", "thumb"}
    _EXCLUDE_NAMES = {"face_mask.png", "reconstruction.pkl", "info.json",
                      "geometry_metrics.json", "texture_metrics.json",
                      "mesh.obj", "mesh.mtl", "uv_texture.png", "uv_confidence.png"}
    return sorted(
        [p for p in root.rglob("*")
         if p.is_file()
         and p.suffix.lower() in IMAGE_SUFFIXES
         and not p.name.startswith("._")
         and p.stem not in _EXCLUDE_STEMS
