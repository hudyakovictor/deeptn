...
class CompareEngine:
    ...
    def build_pairwise_evidence(self, main_root: str | Path, reference_path: str | Path | None = None) -> list[PairEvidence]:
        ...
        if not records:
            log.warning(f"No stage2 records found for comparison in {root}")
            return []
        ...
        for bucket, bucket_records in grouped.items():
            ...
            # Adjacent pairs
            total_pairs += max(0, n - 1) * window
            # Anchor pairs
            anchors = self._select_anchors(bucket_records, stage1_records)
            total_pairs += len(anchors) * n
        
        log.info(f"Building pairwise evidence for {len(records)} photos (~{total_pairs} pairs)")
        
        progress = create_progress(total=total_pairs, description="S4 Compare")
            ...
            for idx, current in enumerate(ordered):
                ...
                for offset in range(1, min(window, idx) + 1):
                    ...
                    try:
                        ...
                        error_count += 1
                        log.error(f"[{a.photo_id}↔{b.photo_id}] Comparison failed: {exc}")
                        progress.update(photo_id=f"{a.photo_id}↔{b.photo_id}", status=f"FAILED", error=True)
                    pairs_processed += 1
                    progress.advance()
                ...
                for anchor in anchors:
                    ...
                    try:
                        ...
                        error_count += 1
                        log.error(f"[{anchor.photo_id}↔{current.photo_id}] Comparison failed: {exc}")
                        progress.update(photo_id=f"{anchor.photo_id}↔{current.photo_id}", status=f"FAILED", error=True)
                    pairs_processed += 1
                    progress.advance()
        ...
        # Summary
        if error_count > 0:
            log.error(f"S4 complete: {len(evidence)} pairs, {error_count} errors")
        else:
            log.success(f"S4 complete: {len(evidence)} pairs ✓")
    ...
    def _load_all_reconstructions(self, root: Path) -> Dict[str, dict]:
        ...
        for path in sorted(root.glob("*/reconstruction.pkl")):
            try:
                ...
                error_count += 1
                log.error(f"Failed to load reconstruction for {path.parent.name}: {e}")
        
        if error_count > 0:
            log.error(f"Loaded {len(reconstructions)} reconstructions, {error_count} failed")
        else:
            log.info(f"Loaded {len(reconstructions)} reconstructions for ICP")
    ...
    def _icp_align_and_compare(self, recon_a: dict, recon_b: dict, 
        ...
        try:
            ...
            log.debug("ICP failed for pair: %s", e)
            return None
