...
class GeometryExtractor:
    ...
    def extract(self, reconstruction: Dict[str, Any]) -> Dict[str, float]:
        ...
        Вершины приводятся к canonical pose для данного bucket перед извлечением.
        
        Важно: canonicalize работает с WORLD vertices (base @ R + trans),
        а НЕ с camera vertices (где to_camera() инвертирует Z нелинейно).
        ...
        # Canonical alignment: rotate to bucket pose before metric extraction.
        # Это ставит модель в "идеально ровное положение" для каждого ракурса:
        # pitch=0, roll=0, yaw=цель для bucket (frontal=0, left_3q=-22.5, и т.д.)
        bucket = reconstruction.get("bucket", "unknown")
        if bucket and bucket != "unknown" and verts.size > 0:
            angles = np.asarray(reconstruction.get("angles_deg", [0.0, 0.0, 0.0]), dtype=np.float64)
            rot_matrix = reconstruction.get("rotation_matrix", None)
            translation = reconstruction.get("translation", None)
            if rot_matrix is not None and len(rot_matrix) > 0:
                rot_matrix = np.asarray(rot_matrix, dtype=np.float64)
            if translation is not None and len(translation) > 0:
                translation = np.asarray(translation, dtype=np.float64)
            try:
                verts = canonicalize_vertices_for_bucket(
