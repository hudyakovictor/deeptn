...
from ..shared.schemas import PipelineDataset, Stage4Record, Stage5Record
...
class ReportEngine:
    ...
    def build_report(self, main_root: str | Path) -> Stage5Record:
        root = Path(main_root)
        ...
        if not verdicts:
            log.warning("No verdicts found for report")
            report = Stage5Record(
        ...
        log.info(f"Building report for {len(verdicts)} verdicts")
        ...
        stage2_records = {}
        error_count = 0
        photo_dirs = [d for d in root.iterdir() if d.is_dir()]
        
        progress = create_progress(total=len(photo_dirs), description="S6 Loading data")
        ...
        for photo_dir in photo_dirs:
            ...
            try:
                progress.update(photo_id=photo_id, status="loading")
                ...
                if info and isinstance(geo, dict):
                    ...
                    stage2_records[info.get("photo_id", photo_id)] = Stage2Record(
                        photo_id=info.get("photo_id", photo_id),
                ...
                error_count += 1
                log.error(f"[{photo_id}] Failed to load stage2 data: {exc}")
        ...
        if error_count > 0:
            log.error(f"Loaded {len(stage2_records)} stage2 records, {error_count} failed")
        ...
        log.success(f"S6 complete: report generated for {len(verdicts)} photos ✓")
        
        return report

    def _load_verdicts(self, root: Path) -> list[Stage4Record]:
        payload = load_json(root / "verdicts.json", default=[]) or []
        return [Stage4Record.model_validate(item) for item in payload]

    def _aggregate_statistics(self, verdicts: list[Stage4Record], timeline: list[dict[str, object]]) -> dict[str, object]:
    ...
    def _build_personas_era_fallback(
        self, timeline: list[dict[str, object]], verdicts: list[Stage4Record]
    ) -> list[dict[str, object]]:
    ...
    def _to_markdown(self, report: Stage5Record) -> str:
        lines = [
    ...
    def _limitations_section(self, report: Stage5Record) -> list[str]:
