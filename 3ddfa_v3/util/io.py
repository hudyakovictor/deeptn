...
class visualize:
    ...
    def visualize_and_output(self, trans_params, img, save_path, img_name):
        ...
        # ITER1: always copy landmarks before Y-flip so result_dict is not mutated
        if 'ldm68' in self.items:
            ldm68 = np.asarray(self.result_dict['ldm68'][0], dtype=np.float64).copy()
        ...
        if 'ldm106' in self.items:
            ldm106 = np.asarray(self.result_dict['ldm106'][0], dtype=np.float64).copy()
        ...
        if 'ldm106_2d' in self.items:
            ldm106_2d = np.asarray(self.result_dict['ldm106_2d'][0], dtype=np.float64).copy()
        ...
        if 'ldm134' in self.items:
            ldm134 = np.asarray(self.result_dict['ldm134'][0], dtype=np.float64).copy()
