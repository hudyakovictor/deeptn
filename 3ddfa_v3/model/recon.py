...
def process_uv(uv_coords, uv_h = 224, uv_w = 224):
    # ITER1: do not mutate caller arrays
    uv = np.asarray(uv_coords, dtype=np.float64).copy()
    uv[:, 0] = uv[:, 0] * (uv_w - 1)
    uv[:, 1] = uv[:, 1] * (uv_h - 1)
    # uv[:,1] = uv_h - uv[:,1] - 1
    uv = np.hstack((uv, np.zeros((uv.shape[0], 1))))  # add z
    return uv
...
class face_model:
    def __init__(self, args):
        ...
        if self.args.ldm106_2d:
            ...
            self.v_parallel = torch.full((35709,), -1, device=self.device, dtype=torch.int64)
    ...
    def to_camera(self, face_shape):
        # ITER1: never mutate input tensor in-place (canonical geometry corruption)
        out = face_shape.clone()
        out[..., -1] = self.camera_distance - out[..., -1]
        return out
    ...
    def forward(self, no_pca=False, identity_only=False, neutral_expression=False, neutral_scale=0.1):
        """Run reconstruction.

        ITER1 extras:
          - always export alpha coefficients (id/exp/alb/angle/sh/trans)
          - optional identity_only / soft-neutral expression meshes
          - camera/crop metadata contract
          - no in-place mutation of intermediate geometry
        """
        ...
        # Expression modes for forensic identity extraction
        exp_tensor = alpha_dict['exp']
        if identity_only:
            exp_tensor = torch.zeros_like(exp_tensor)
        elif neutral_expression:
            exp_tensor = exp_tensor * float(neutral_scale)

        face_shape = self.compute_shape(alpha_dict['id'], exp_tensor)
        face_shape_identity = self.compute_shape(
            alpha_dict['id'], torch.zeros_like(alpha_dict['exp'])
        )
        ...
        face_norm_identity = self.compute_norm(face_shape_identity)
        ...
        angles_deg = np.rad2deg(alpha_dict['angle'].detach().cpu().numpy())

        result_dict = {
            'v3d': v3d.detach().cpu().numpy(),
            'v2d': v2d.detach().cpu().numpy(),
            # ITER1: model-space / identity meshes (no camera)
            'v3d_model': face_shape.detach().cpu().numpy(),
            'v3d_identity': face_shape_identity.detach().cpu().numpy(),
            'v3d_transformed': face_shape_transformed.detach().cpu().numpy(),
            'normals_model': face_norm.detach().cpu().numpy(),
            'normals_identity': face_norm_identity.detach().cpu().numpy(),
            'normals_camera': face_norm_roted.detach().cpu().numpy(),
            'rotation_matrix': rotation.detach().cpu().numpy(),
            ...
            # ITER1: full alpha export (previously discarded)
            'alpha_raw': alpha.detach().cpu().numpy(),
            'alpha_id': alpha_dict['id'].detach().cpu().numpy(),
            'alpha_exp': alpha_dict['exp'].detach().cpu().numpy(),
            'alpha_exp_used': exp_tensor.detach().cpu().numpy(),
            'alpha_alb': alpha_dict['alb'].detach().cpu().numpy(),
            'alpha_angle': alpha_dict['angle'].detach().cpu().numpy(),
            'alpha_angle_deg': angles_deg,
            'alpha_sh': alpha_dict['sh'].detach().cpu().numpy(),
            'alpha_trans': alpha_dict['trans'].detach().cpu().numpy(),
            # ITER1: camera / crop contract
            'camera': {
                ...
                'projection_matrix_3x3': self.persc_proj.detach().cpu().numpy(),
            },
            'trans_params': None if self.trans_params is None else np.asarray(self.trans_params),
            'expression_mode': (
                'identity_only' if identity_only
                else ('neutral_soft' if neutral_expression else 'full')
            ),
            'neutral_scale': float(neutral_scale) if neutral_expression else None,
            'schema_version': '3ddfa_v3_iter1_v1',
            'coordinate_spaces': {
                'v3d': 'camera',
                'v3d_model': 'model_with_expression',
                'v3d_identity': 'model_identity_zero_expression',
                'v3d_transformed': 'model_posed_translated',
                'v2d': 'crop_224_y_up_image_plane',
                'units': 'bfm_model_units',
            },
        }

        # compute visible vertice according to normal and renderer
        # ITER1: always compute visible_idx; keep tensor for internal use, export numpy
        visible_idx = torch.zeros(35709, dtype=torch.int64, device=v3d.device)
        if visible_idx_renderer is not None:
            idx = visible_idx_renderer.type(torch.int64).reshape(-1)
            idx = idx[(idx >= 0) & (idx < 35709)]
            visible_idx[idx] = 1
